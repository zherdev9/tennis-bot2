...
async def send_invite_callback(callback: CallbackQuery):
    ...
    # Проверим, не является ли игрок уже участником матча
    participant_ids = await get_game_participant_ids(game_id, include_creator=True)
    if invited_id in participant_ids:
        await callback.answer("Этот игрок уже участвует в матче.", show_alert=True)
        return

    # Получаем данные приглашаемого игрока (для красивого текста организатору)
    invited_user = await get_user(invited_id)

    # Формируем карточку матча для приглашённого
    ...
    # Время матча с учётом длительности
    ...
    try:
        ...
        # Покажем организатору понятное сообщение в чат,
        # иначе он видит только маленький алерт и думает,
